  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(600px circle at ${(pointer.x + 0.5) * 100}% ${(pointer.y + 0.5) * 100}%, color-mix(in oklab, var(--primary) 10%, transparent), transparent 65%)`,
        }}
      />
      ...
      <div className="absolute inset-0 hidden xl:block pointer-events-none">
        {floatCards.map((card) => (
          <motion.div
            key={card.title}
            initial={{ opacity: 0, y: 40, rotate: card.depth > 0 ? -4 : 4 }}
            animate={{
              opacity: 1,
              y: pointer.y * card.depth,
              x: pointer.x * card.depth,
